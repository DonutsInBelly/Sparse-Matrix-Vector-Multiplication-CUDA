...
void spmv_atomic_kernel(const int nnz, const int * coord_row, const int * coord_col, const float * A, const float * x, float * y){
...
void segmented_scan(const int lane, const int * rows, float * vals){

	if ( lane >= 1 && rows[threadIdx.x] == rows[threadIdx.x − 1] ) vals [ threadIdx .x] += vals [ threadIdx .x − 1];
	if ( lane >= 2 && rows[threadIdx.x] == rows[threadIdx.x − 2] ) vals [ threadIdx .x] += vals [ threadIdx .x − 2];
	if ( lane >= 4 && rows[threadIdx.x] == rows[threadIdx.x − 4] ) vals [ threadIdx .x] += vals [ threadIdx .x − 4];
	if ( lane >= 8 && rows[threadIdx.x] == rows[threadIdx.x − 8] ) vals [ threadIdx .x] += vals [ threadIdx .x − 8];
	if ( lane >= 16 && rows[threadIdx.x] == rows[threadIdx.x − 16] ) vals [ threadIdx .x] += vals [ threadIdx .x − 16];
}
